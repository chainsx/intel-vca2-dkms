...
bool data_field::string(char const*){
	return true;
}
bool data_field::number(char const* t){
	while(( '0'<= *t)&&( *t<= '9')) ++t;
	return 0== *t;
}
bool data_field::ip(char const*){
	return true;
}
bool data_field::mac(char const*){
	return true;
}
bool data_field::file(char const* t){
	return '/'== *t;
}

data_field::data_field(): name(""), validator(data_field::string) {}
data_field::data_field( bool(*validator)(char const*),char const* name_,const char*value): name(name_), validator(validator), data(value) {}

data_field& data_field::operator=(const data_field& s) {
	LOG_DEBUG("Why duplicates data %s %s?\n", name, data.c_str());
	data= s.data;
	(char const*&)name= s.name;
	(bool(*&)(char const*))validator= s.validator;
	return *this;
}
...
global_fields::global_fields()
	:auto_boot(data_field::number, "auto-boot", "1")
	,debug_enabled(data_field::number, "debug-enabled", "0")
	,link_up_timeout_ms(data_field::number, "link-up-timeout-ms", "2000")
	,handshake_irq_timeout_ms(data_field::number, "handshake-irq-timeout-ms", "30000")
	,alloc_timeout_ms(data_field::number, "alloc-timeout-ms", "100")
	,cmd_timeout_ms(data_field::number, "cmd-timeout-ms", "1000")
	,mac_write_timeout_ms(data_field::number, "mac-write-timeout-ms", "1000")
	,default_daemon_script(data_field::string, "default-daemon-script", "")
	,wait_cmd_timeout_s(data_field::number, "wait-cmd-timeout-s", "200")
	,wait_bios_cmd_timeout_s(data_field::number, "wait-bios-cmd-timeout-s", "300")
	,wait_bios_cmd_flashing_s(data_field::number, "wait-bios-cmd-flashing-s", "1200")
	,ICMP_ping_interval_s(data_field::number, "ICMP-ping-inverval-s", "1")
	,ICMP_response_timeout_s(data_field::number, "ICMP-response-timeout-s", "10")
	,va_min_free_memory_enabled(data_field::number, "va-min-free-memory-enabled", "1")
{}

data_field const* global_fields::operator [](unsigned i) const {
	switch( i){
	case 0: return& auto_boot;
	case 1: return& debug_enabled;
	case 2: return& link_up_timeout_ms;
	case 3: return& handshake_irq_timeout_ms;
	case 4: return& alloc_timeout_ms;
	case 5: return& cmd_timeout_ms;
	case 6: return& mac_write_timeout_ms;
	case 7: return& default_daemon_script;
	case 8: return& wait_cmd_timeout_s;
	case 9: return& wait_bios_cmd_timeout_s;
	case 10:return& wait_bios_cmd_flashing_s;
	case 11:return& ICMP_ping_interval_s;
	case 12:return& ICMP_response_timeout_s;
	case 13:return& va_min_free_memory_enabled;
	}
	return NULL;
}
...
{
	if(data.validator(val.c_str())) {
		data.set_value( val.c_str());
		return true;
	}
	LOG_ERROR("%s is invalid for %s\n", val.c_str(), data.name);
	return val.empty();
}
...
{
	for(unsigned i= 0; data_field const*const f=(*this)[ i]; ++i)
		if( strcmp( field_name, f->name) == 0)
			return true;
	return false;
}
...
bool vca_config::parse_global(boost::property_tree::ptree node)
{
	try {
		BOOST_FOREACH(boost::property_tree::ptree::value_type const& v, node.get_child("global")) {
			for (unsigned i= 0; data_field* f=(data_field*)(*this)[i]; ++i) {
				if (v.first == f->name) {
					if (!try_set_data_field(v.second.data(), *f))
						return false;
					break;
				}
			}
...
std::string get_default_cpu_ip(int card_id, int cpu_id)
{
	return std::string("172.31.") + int_to_string(1 + card_id * MAX_CPU + cpu_id) + ".1";
}
...
std::string get_default_host_ip(int card_id, int cpu_id)
{
	return std::string("172.31.") + int_to_string(1+ card_id * MAX_CPU + cpu_id) + ".254";
}
...
std::string get_default_node_name(int card_id, int cpu_id)
{
	return std::string("vca_node_") + int_to_string (card_id) +
		int_to_string (cpu_id);
}

std::string get_default_nfs_server(int card_id, int cpu_id)
{
	return get_default_host_ip(card_id, cpu_id);
}
...
bool vca_config::save_default_config()
{
	...
	for (unsigned i = 0; const data_field* field=(*this)[i]; ++i) {
		global.put(field->name, field->get_cstring());
	}
...
{
	return config->auto_boot.get_number() == 1 ? true : false;
}

bool vca_config_is_va_min_free_memory_enabled(struct vca_config *config)
{
	return !!config->va_min_free_memory_enabled.get_number();
}
