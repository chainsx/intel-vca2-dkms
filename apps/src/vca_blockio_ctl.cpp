...
unsigned max_blk() {
	if(close_on_exit f= open( "/sys/module/vcablk_bckend/parameters/vcablk_bcknd_max_dev", O_RDONLY)) {
		char t[ 16], *const e= t+ read( f, t, sizeof( t)- 1);
		if( t< e) {
			*e= 0;
			unsigned const n= atoi( t);
			return ( n<= MAX_BLK_DEVS)? n: MAX_BLK_DEVS;
		}
	}
	return MAX_BLK_DEVS;
}
...
bool extract_block_dev_id(const char *device_str, int &id) {
	bool ok = device_str
		&& (sscanf(device_str, BLK_CONFIG_NAME "%u", &id) == 1)
		&& id < MAX_BLK_DEVS;
	if (!ok) id = FAIL;
	return ok;
}
...
blk_exist check_blk_disk_exist(filehandle_t blk_dev_fd, unsigned short blk_dev_id)
{
	struct vcablk_disk_info_desc info= { blk_dev_id, Blk_error };
	int const rc = ioctl( blk_dev_fd, VCA_BLK_GET_DISK_INFO, &info);
	if( 0<= rc)
		return static_cast<blk_exist>( info.exist);
