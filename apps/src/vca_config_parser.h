...
struct data_field {
public:
	static bool string(char const*);
	static bool number(char const*);
	static bool ip(char const*);
	static bool mac(char const*);
	static bool file(char const*);
	const char* const name;
	bool(*const validator)(char const*);
private:
	std::string data;
public:
	data_field();
	data_field( bool(*validator)(char const*),char const* name_,const char*value);
	unsigned int get_number() const { return atoi(data.c_str()); }
	...
	data_field& operator=(const data_field&);
};

struct global_fields {
	global_fields();
	data_field auto_boot;
	data_field debug_enabled;
	data_field link_up_timeout_ms;
	data_field handshake_irq_timeout_ms;
	data_field alloc_timeout_ms;
	data_field cmd_timeout_ms;
	data_field mac_write_timeout_ms;
	data_field default_daemon_script;
	data_field wait_cmd_timeout_s;
	data_field wait_bios_cmd_timeout_s;
	data_field wait_bios_cmd_flashing_s;
	data_field ICMP_ping_interval_s;
	data_field ICMP_response_timeout_s;
	data_field va_min_free_memory_enabled;
	data_field const *operator[](unsigned) const;
};

struct vca_config: public global_fields {
private:
	std::string filename;
