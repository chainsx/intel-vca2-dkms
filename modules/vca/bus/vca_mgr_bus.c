...
static int vca_mgr_dev_match(struct device *dv, const struct device_driver *dr)
#else
static int vca_mgr_dev_match(struct device *dv, struct device_driver *dr)
#endif
{
	unsigned int i;
	struct vca_mgr_device *dev = dev_to_vca_mgr((struct device *)dv);
	const struct vca_mgr_device_id *ids;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 11, 0)
	ids = container_of(dr, const struct vca_mgr_driver, driver)->id_table;
#else
...
static int vca_mgr_uevent(const struct device *dv, struct kobj_uevent_env *env)
{
	struct vca_mgr_device *dev = dev_to_vca_mgr((struct device *)dv);
...
static void vca_mgr_dev_remove(struct device *d)
{
	struct vca_mgr_device *dev = dev_to_vca_mgr(d);
	struct vca_mgr_driver *drv = drv_to_vca_mgr(dev->dev.driver);
