...
struct vcablk_disk {
	...
	struct blk_mq_tag_set tag_set;     /* blk-mq tag set for Linux 6.x */
	bool tag_set_initialized;
	bool gdisk_started;
	bool read_only;
	int dev_id;
	...
	/* wait queue for release request_ring */
	wait_queue_head_t	request_ring_wait;
	...
	/* Remember error of part split request */
	__u16 split_request_error;
...
{
	blk_status_t status = ret ? BLK_STS_IOERR : BLK_STS_OK;
...
#define GBIOVP(bvec) (bvec)
	...
	if (req_op(req) == REQ_OP_FLUSH ||
	    (req->cmd_flags & (REQ_SYNC | REQ_PREFLUSH | REQ_FUA)))
		do_sync = true;
#else
	...
	rq_for_each_segment(bvec, req, iter) {
		...
		ret = vcablk_disk_request_step(dev, GBIOVP(bvec), bio_context,
				request, sector, sectors_num, stop_f);
...
static blk_status_t
vcablk_disk_queue_rq(struct blk_mq_hw_ctx *hctx,
		const struct blk_mq_queue_data *bd)
{
	struct request *req = bd->rq;
	struct vcablk_disk *dev = hctx->queue->queuedata;
	unsigned long flags;
	int new_head;
	...
	spin_lock_irqsave(&dev->req_ring_lock, flags);
	new_head = (dev->req_ring.head + 1) & REQ_RING_MASK;
	if (new_head == dev->req_ring.tail) {
		spin_unlock_irqrestore(&dev->req_ring_lock, flags);
...
static const struct blk_mq_ops vcablk_mq_ops = {
	.queue_rq = vcablk_disk_queue_rq,
};
...
static int
vcablk_disk_open(struct gendisk *disk, blk_mode_t mode)
{
...
static void
vcablk_disk_release(struct gendisk *disk)
{
	struct vcablk_disk *dev = disk->private_data;
	pr_debug("%s:\n", __func__);
	spin_lock_irq(&dev->lock);
...
vcablk_disk_stop(struct vcablk_disk *dev, bool force)
{
	...
	if (dev->tag_set_initialized) {
		blk_mq_free_tag_set(&dev->tag_set);
		dev->tag_set_initialized = false;
	}
...
static int
vcablk_disk_ioctl (struct block_device *bdev, blk_mode_t mode,
 ...
 */
	...
	if (err) {
		pr_err("%s: blk_mq_alloc_tag_set failure %d\n", __func__, err);
		goto err;
	}
	dev->tag_set_initialized = true;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
	{
		...
		disk = dev->gdisk = blk_mq_alloc_disk(&dev->tag_set, &lim, dev);
	}
#else
	disk = dev->gdisk = blk_mq_alloc_disk(&dev->tag_set, dev);
#endif
	if (IS_ERR(disk)) {
		err = PTR_ERR(disk);
		dev->gdisk = NULL;
		pr_err("%s: blk_mq_alloc_disk failure %d\n", __func__, err);
		goto err;
	}
	dev->queue = disk->queue;
	...
	/* Explicit major requires an explicit minor range. */
	disk->minors = VCA_BLK_MINORS;
...
vcablk_disk_start(struct vcablk_disk *dev)
{
	...
	err = add_disk(disk); /* THIS FUNCTION BLOCK context for few REQUESTS*/
	if (err) {
