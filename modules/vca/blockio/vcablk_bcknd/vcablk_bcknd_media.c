...
{
	return filp_open(path, flags | O_LARGEFILE, rights);
}
		...
		unsigned long nsect, char *buffer, int write)
{
	...
	if (write) {
		memcpy(media->data.memory + offset, buffer, nbytes);
	} else {
		memcpy(buffer, media->data.memory + offset, nbytes);
	}
		...
		unsigned long nsect, char *buffer, int write)
{
	...
	if (!likely(nbytes == nbytesdone)) {
		pr_warn("%s: %s ERROR nbytes %lu nbytesdone %li\n",
