...
static void vcablk_bcknd_buffer_deinit(vcablk_pool_t *vcablk_buffer_pool, struct dma_chan *dma_ch)
{
	vcablk_buffer *buffer;
	void* ptr = NULL;
	int iter = 0;
...
static int vcablk_bcknd_buffer_init(struct dma_chan *dma_ch, vcablk_pool_t *vcablk_buffer_pool)
{
...
static int vcablk_bcknd_transfer(struct vcablk_bcknd_disk *bckd,
		unsigned long sector,
		...
		struct vcablk_request *request_buff, __u16 size)
{
	...
	if (sync_before) {
		++id;
		WARN_ONCE(1, "%s: SYNC not fully implemented for ASYNC DMA!\n", __func__);
