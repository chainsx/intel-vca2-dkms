 ...
 *
 * */
static void plx_a_lut_disable(struct plx_device *xdev)
{
 ...
 * RETURNS: 0 on success and errno on failure
 */
static int plx_check_eeprom(struct plx_device *xdev)
{
	u32 val;
	u32 crc;
 ...
 * RETURNS: true if link is up, false otherwise
 */
static u32 __maybe_unused plx_get_state(struct plx_device *xdev)
{
 ...
 * @cpu_id - cpu id
 */
static void plx_turn_rcv_mode(struct plx_device *xdev, u32 cpu_id, bool turn_on)
{
	if (xdev->card_type & VCA_MV) {
		if (turn_on)
