...
static void plx_unregister_device(struct plx_device *xdev)
{
	if (!xdev->link_side) {
		if (xdev->vca_csm_dev) {
			...
			xdev->vca_csm_dev = NULL;
		}
		if (xdev->vca_mgr_dev) {
			vca_mgr_unregister_device(xdev->vca_mgr_dev);
			xdev->vca_mgr_dev = NULL;
		}
		if (xdev->vca_mgr_extd_dev) {
			vca_mgr_extd_unregister_device(xdev->vca_mgr_extd_dev);
			xdev->vca_mgr_extd_dev = NULL;
		}
#if VCA_BUILD_BLOCKIO
