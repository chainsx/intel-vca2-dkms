...
static inline const char *get_vca_os_type_string(enum vca_os_type os_type)
{
	switch (os_type) {
 ...
 */
static inline const char *get_vca_state_string(enum vca_lbp_states state)
{
	switch(state) {
		case VCA_BIOS_DOWN:
...
static inline const char *get_vca_rcvy_state_string(enum vca_lbp_rcvy_states rcvy_state)
{
	switch (rcvy_state)
	{
