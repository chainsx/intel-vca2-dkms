 ...
 */
static int vop_wait_for_completion(struct vop_dev_common *cdev, struct completion *x,
	    int timeout_ms)
{
	long res;
 ...
 */
static dma_cookie_t vop_async_dma(struct vop_device *vpdev, dma_addr_t dst,
 ...
 */
static int __maybe_unused vop_sync_dma(struct vop_device *vpdev, dma_addr_t dst,
...
static int sync_descriptors_dma_task(void *data)
{
...
static void sync_descriptors_read_task_step(struct vop_dev_common *cdev)
{
	struct vop_device *vdev = cdev->vdev;
...
/* removed build-time pragma message for warning-clean Linux 6.8 build */
